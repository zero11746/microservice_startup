...
type Register struct {
	...
	srvTTL  int64
	cli     *clientv3.Client
	logger  *applog.Tracer
}

// NewRegister create a register base on etcd
func NewRegister(etcdAddrs []string, logger *applog.Tracer) *Register {
	return &Register{
		EtcdAddrs:   etcdAddrs,
		DialTimeout: 3,
		logger:      logger,
...
// Register a service
func (r *Register) Register(srvInfo Server, ttl int64) (*Register, chan<- struct{}, error) {
	var err error
	...
	for {
		...
		case <-r.closeCh:
			if err := r.unregister(); err != nil {
				r.logger.Error("unregister failed", err)
			}
			if _, err := r.cli.Revoke(context.Background(), r.leasesID); err != nil {
				r.logger.Error("revoke failed", err)
			}
			return
